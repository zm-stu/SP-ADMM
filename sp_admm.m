function [rmse_sp,S_sum,U_sum,P_sum,esti] = sp_admm(agent,sum_link,link,c,rho,u0,a0,x0,step,n_fix,m,dd_error)
...
for i = 1:n_fix+m
    ...
    u{1,i} = u0.*ones(a*2,1);
    QTQ{i,1} = Q'*Q;
    s_temp=QTQ{i,1}*x{1,i}-QTD{i,1}*u{1,i}+alpha{1,i};
    S_sum(1)=S_sum(1)+s_temp'*s_temp;
end
...
for t = 2:step
    tic;   
        ...
        for i = 1:n_fix+m
             ...
             % gap record
               P_sum(t)=P_sum(t)+x{1,i}'*ATA{i,1}*x{1,i};
               U_sum(t)=U_sum(t)+(u{1,i}-u_old)'*(u{1,i}-u_old);
               s_temp=QTQ{i,1}*x{1,i}-QTD{i,1}*u{1,i}+alpha{1,i};
               S_sum(t)=S_sum(t)+s_temp'*s_temp;
        end
